...
@Component({
  ...
  styleUrls: ['./passenger-trips.page.scss'],
})
export class PassengerTripsPage implements OnInit {
  ...
  selectedStatus: string = 'pendiente';  // Valor inicial del estado del filtro
  ...
  ngOnInit() {
    ...
    if (userData) {
      ...
      this.loadPassengerTrips(userId);
    }
  }
  ...
  onStatusChange(event: any) {
    ...
    if (userData) {
      ...
      this.loadPassengerTrips(userId, this.selectedStatus);
    }
  }

  // Función para cargar los viajes solicitados por el pasajero
  loadPassengerTrips(userId: number, statusTrip: string = 'pendiente') {
    const params = {
      userIdCreator: userId,
      statusTrip: statusTrip  // Se puede ajustar este parámetro
    };

    this.generalService.get('api/tripsByTripCreator', params).subscribe({
      next: (response: any) => {
        this.trips = response;
  ...
  submitCalification(trip: any) {
    ...
    if (userData) {
      ...
      const userId2 = trip.route.person.userId;  // Conductor (el calificado)
      ...
      const calificationData = {
        ...
        FK_userId2: userId2,  // ID del conductor
