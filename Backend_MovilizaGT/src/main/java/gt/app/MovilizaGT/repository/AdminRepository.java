...
@Repository
public interface AdminRepository extends JpaRepository<Trip, Integer> {
    ...
    @Query(value = "SELECT * FROM trip  " +
            "WHERE FK_userId = :userIdCreator " +
            "AND statusTrip = :statusTrip " +
            "ORDER BY tripStart DESC", nativeQuery = true)
